	...
	while (true)
	{
		...
		if (flag)
		{
			if (0)
			{
				...
				CalcCorners(H, image1);
				cout << H << endl;
				fstream f("try.txt", ios::out);//供写使用，文件不存在则创建，存在则清空原内容
			...
			else
			{
				...
				ifstream ifile1("try.txt", ios::in);
				if (!ifile1)
					cerr << "error_O_L" << endl;
				string lineword;
				int j = 0;
				...
				while (ifile1 >> lineword)
				{
					...
					//sscanf_s(lineword.c_str(), "%f", &temp);//读取到x_L数组里面
					//*data++ = temp;
					//printf("%lf\n", x_L[j]);
					j++;
					/*if (j % 3 == 0 && j != 0)
		...
		try
		{
			...
			warpPerspective(image1, imageTransform1, H, Size(MAX(corners.right_top.x, corners.right_bottom.x), image2.rows));
